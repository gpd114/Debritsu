...
/**
 * M PLUS Rounded 1c, the phone's face and the icon's wordmark, in the same
 * Latin-only cuts. Anything outside them falls back to the system font.
 */
val Rounded = FontFamily(
    Font("font/mplus_rounded_medium.ttf", FontWeight.Medium),
    Font("font/mplus_rounded_bold.ttf", FontWeight.Bold),
    Font("font/mplus_rounded_extrabold.ttf", FontWeight.ExtraBold)
)

/** Downloaded files: semantic, and deliberately not the accent. */
val Keep: Color get() = if (Ink.palette.dark) Color(0xFF6FC79B) else Color(0xFF2E8A5F)

/** Filler, recaps and failures: worth marking in a colour that is not the accent. */
val Warn: Color get() = if (Ink.palette.dark) Color(0xFFE29075) else Color(0xFFB5532F)
...
private val Type = Typography(
    headlineSmall = TextStyle(fontFamily = Rounded, fontWeight = FontWeight.ExtraBold, fontSize = 24.sp, letterSpacing = (-0.4).sp),
...
/**
 * The main button — Play, Sign in, Save — in the theme's action colour, as on
 * the phone. Material's own default is the accent, which on the dark themes is
 * a light purple that white text cannot be read against.
 */
@Composable
fun PrimaryButton(
    onClick: () -> Unit,
    ...
    colors = androidx.compose.material3.ButtonDefaults.buttonColors(
        containerColor = Ink.palette.action,
        contentColor = Color.White,
        disabledContainerColor = Ink.Quiet,
