 ...
 *
 * Handing the URL to a browser is the usual approach and useless here: most
 * televisions carry none. Android still resolves the intent, because the
 * platform ships a stub activity whose whole job is to answer with "no app can
 * perform this action" — so the failure looks like a broken button rather than
 * a missing browser, and asking whether anything can handle the intent does not
 * tell the two apart.
 *
 * A WebView is present even where a browser is not, so this hosts the flow
 * itself and watches for the redirect rather than relying on a deep link
 * arriving back through the launcher. This build always takes this path; there
 * is no browser to prefer.
 */
